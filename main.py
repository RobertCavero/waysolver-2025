...
def main():
    ...
    obstacle_mode = False
    mouse_held = False
    algorithm_generator = None  # 🔁 Added generator variable
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.USEREVENT and event.user_type == pygame_gui.UI_BUTTON_PRESSED:
                if event.ui_element == start_button and start and end:
                    ...
                    start = end = None
                    grid = make_grid(ROWS, GRID_SIZE)
                    algorithm_generator = None  # Reset the algorithm if it's running
...
def gerar_labirinto(grid, start, end):
    for row in grid:
        for spot in row:
            # Só gera barreiras se start e end estiverem definidos
            if (not spot.is_start()) and (not spot.is_end()):
...
def carregar_mapa_de_arquivo(nome_arquivo, grid):
    ...
    for i, linha in enumerate(linhas):
        for j, char in enumerate(linha):
            if j < len(grid) and i < len(grid[j]):  # Inverter i <-> j
                spot = grid[j][i]  # Corrige o espelhamento apenas na leitura
                if char == 'S':
                    spot.make_start()
                    start = spot
                elif char == 'E':
